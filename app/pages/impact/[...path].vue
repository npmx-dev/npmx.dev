...
<template>
  <div class="impact-page flex-1 flex flex-col">
    ...
    <header
      ...
      class="sticky top-14 z-10 bg-bg/95 backdrop-blur border-b border-border"
    >
    ...
    <div v-if="!resolvedVersion" class="container py-20 text-center">
      ...
      <NuxtLink :to="`/${packageName}`" class="btn">{{ $t('impact.go_to_package') }}</NuxtLink>
    </div>

    <!-- Main content - client only since it uses Web Workers -->
    <ClientOnly v-else>
      <ImpactAnalyzer :key="packageSpec" :package-spec="packageSpec!" :package-name="packageName" />
      <template #fallback>
