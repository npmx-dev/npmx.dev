...
<template>
  <section id="maintainers" v-if="maintainers?.length" class="scroll-mt-20">
    ...
    <ul class="space-y-2 list-none m-0 p-0" :aria-label="$t('package.maintainers.list_label')">
      ...
      >
        ...
        <button
          ...
          type="button"
          class="p-1 text-fg-subtle hover:text-red-400 transition-colors duration-200 shrink-0 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
          :aria-label="
    ...
    <button
      ...
      type="button"
      class="mt-2 text-xs text-fg-muted hover:text-fg transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50 rounded"
    ...
    <div v-if="canManageOwners" class="mt-3">
      <div v-if="showAddOwner">
        <form class="flex items-center gap-2" @submit.prevent="handleAddOwner">
          ...
          <input
            ...
            v-bind="noCorrect"
            class="flex-1 px-2 py-1 font-mono text-sm bg-bg-subtle border border-border rounded text-fg placeholder:text-fg-subtle transition-colors duration-200 focus:border-border-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
          />
          <button
            type="submit"
            :disabled="!newOwnerUsername.trim() || isAdding"
            class="px-2 py-1 font-mono text-xs text-bg bg-fg rounded transition-all duration-200 hover:bg-fg/90 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
          >
            {{ isAdding ? '…' : $t('package.maintainers.add_button') }}
          </button>
          <button
            type="button"
            class="p-1 text-fg-subtle hover:text-fg transition-colors duration-200 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
      ...
      <button
        v-else
        type="button"
        class="w-full px-3 py-1.5 font-mono text-xs text-fg-muted bg-bg-subtle border border-border rounded transition-colors duration-200 hover:text-fg hover:border-border-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
