...
<template>
  <div>
    ...
    />
    <label
      class="bg-bg-muted text-fg-muted border-border hover:(text-fg border-border-hover) inline-flex items-center px-2 py-0.5 text-xs font-mono border rounded transition-colors duration-200 outline-none peer-focus-visible:(ring-2 ring-accent/90) border-none peer-checked:(bg-fg text-bg border-fg hover:(text-text-bg/50)) peer-disabled:(opacity-50 pointer-events-none)"
      :htmlFor="internalId"
    >
      <slot />
    </label>
