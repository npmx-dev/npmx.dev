...
- **Install size** - Total size including transitive dependencies, as they would be resolved if this version was installed today. The timeline's **frozen history** toggle, on by default there, instead limits dependencies to the versions that were available before the next version of this package was released, effectively freezing it at that time.
